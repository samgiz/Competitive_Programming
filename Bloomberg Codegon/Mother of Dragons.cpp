...
int maxdragons(int cur, int last, int d){
	// cerr<<last<<" "<<cur<<" "<<d<<endl;
	if(cur==n)return 0;
	if(last>=d){
		return maxdragons(cur+1, Pits[cur+1]-Pits[cur], d)+1;
	}
	else return maxdragons(cur+1, last+Pits[cur+1]-Pits[cur], d);
}
...
int main(){
	...
	while(a!=b){
		int mid = (a+b+1)/2;
		// cerr<<a<<" "<<b<<endl;
		if(a==3)cerr<<maxdragons(0, Pits[1]-Pits[0], 4)<<endl;
		if(maxdragons(1, Pits[1]-Pits[0], mid)>=c-1)a = mid;
		else b = mid-1;
	}
	// cerr<<maxdragons(1, Pits[1]-Pits[0], 5);
	cout<<a;
}
