...
int main(){
	...
	for(int test=0; test<t; test++){
		...
		// Sort dogs by positions
		sort(dogs, dogs+n, [](Dog a, Dog b){return a.p < b.p;});
		// int m = dogs[k-1].p;
		// int M = m*2;
		int ct = 1;
		P[0] = dogs[0].p;
		forii(1, n){
			if(P[ct-1]!=dogs[i].p)P[ct++] = dogs[i].p;
		}
		// P contains all possible positions of the dogs in sorted order. ct is number of positions
		start = 0;
		// Look for starting position
		while(P[start] < dogs[i].p)start++;

		// Add map for all colours
		?

		// Calculate the number of each element up to each position
		int cur = 0;
		fori(n){
			if(dogs[i].p != P[cur]){
				forj(cols){
				...
				sort(Cum[cur], Cum[cur] + cols, greater<int>);
				cur++;
			}
			Cols[M[dogs[i].c]]++;
		}
