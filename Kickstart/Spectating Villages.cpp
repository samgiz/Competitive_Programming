...
void dfs(int prev, int cur){
    ...
    for(auto i: E[cur]){
        if(i == prev)continue;
        dfs(cur, i);
        Nil[cur] += max(FakeIl[i], Nil[i]);
        FakeIl[cur] += max(FakeIl[i], max(Nil[i], Il[i]));
        f = f | (Il[i] >= max(Nil[i], FakeIl[i]));
        x = min(x, max(Nil[i], FakeIl[i]) - Il[i]);
        Il[cur] += max(FakeIl[i], max(Nil[i] + A[i], Il[i]));
    }
    if(!f){
        // One of the children needs to be changed from non-illuminating to illuminating
        FakeIl[cur] -= x;
    }
}
...
int main(){
    ...
    fori1(t){
        ...
        dfs(0, 1);
        cout<<max(max(Il[1], Nil[1]), FakeIl[1])<<endl;
        // fori1(n)cerr<<Il[i]<<" "<<Nil[i]<<" "<<FakeIl[i]<<endl;
    }
}
