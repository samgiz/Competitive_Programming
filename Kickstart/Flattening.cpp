...
int A[N];
int K[N];
// position, block number, position value => min number of changes to the left
int DP[N][N][N];
int min1[N];
    ...
    fori1(t){
        ...
        fori1(n-1){
            forj1(k+1){
                ...
                forl(ct){
                    DP[i][j][l] = min1[j-1];
                    if(DP[i-1][j][l] != -1) DP[i][j][l] = min(DP[i][j][l], DP[i-1][j][l]);
                    DP[i][j][l] += (A[i] != K[l]);
                }
            }
            forj1(k+1)min1[j] = 1e9;
            forj1(k+1){
                forl(ct){
                    if(DP[i][j][l] != -1)min1[j] = min(min1[j], DP[i][j][l]);
                }
            }
            // forj1(k+1)cerr<<min1[j]<<" ";cerr<<endl;
        }
        int ans = 1e9;
        fori1(k+1){
            ans = min(ans, min1[i]);
