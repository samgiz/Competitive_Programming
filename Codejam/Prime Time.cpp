...
vector<ll>amounts;
int ct = 0;
void find_max(int cur, ll sum, ll prod){
    ct += 1;
    if(ct % 10000000 == 0){
        debug(ct);
    }
    // debug(cur, primes, amounts, sum, prod);
    if(sum == prod) {
        max_score = max(max_score, sum);
        return;
    }
    if(amounts[cur] == 0)cur++;
    for(int i=cur; i < (int)primes.size(); i++){
        if(prod > (sum - primes[i]) / primes[i])
            break;
        amounts[i] -= 1;
        find_max(i, sum - primes[i], prod * primes[i]);
        amounts[i] += 1;
        if(max_score != 0)
            return;
    }
...
int main(){
    ...
    fork(t){
        ...
        if(m >= 2 && primes[0] == 2 && primes[1] == 3){
            swap(primes[0], primes[1]);
            swap(amounts[0], amounts[1]);
        }
        find_max(0, sum, 1);
        cout << max_score << endl;
        max_score = 0;
