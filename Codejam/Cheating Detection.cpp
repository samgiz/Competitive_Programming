...
int main(){
    ...
    fork(t){
        ...
        vector<string>s(100);
        fori(100)cin>>s[i];
        vector<int> consensus(10000), sum(100);
        fori(100){
            forj(10000){
                consensus[j] += (s[i][j] == '1');
                sum[i] += (s[i][j] == '1');
        ...
        forj(10000){
            if(consensus[j] < lim){
                cnt++;
                // find the people who disagree
                fori(100){
                    if(s[i][j] == '1')
                        score[i]+= (100 - consensus[j]) * (100 - consensus[j]);
                }
            }
            if(consensus[j] > 100 - lim){
                cnt++;
                // find the people who disagree
                fori(100){
                    if(s[i][j] == '0')
                        score[i]+= consensus[j] * consensus[j];
                }
            }
        }
        debug(score);
        debug(cnt);
        debug(score[40], score[58], score[59], score[99]);
        debug(sum);
        int answer = distance(score.begin(), max_element(score.begin(), score.end())) + 1;
        cout << answer << endl;
    }
}
