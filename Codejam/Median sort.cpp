...
void communicate(string s){
    ...
    // debug("response done");
    if(res == -1){
        debug("response -1 received");
        debug(iterations);
        exit(0);
    }
}

void communicate(int a, int b, int c){
    iterations++;
    // debug("print start");
    cout << a << " " << b << " " << c << endl;
    // debug("print done");
...
int find_index(int new_el, int l, int r, vector<int>& answer){
    if(l+1 == r){
        return r;
    }
    int new_l = l + (r-l+1)/3;
    int new_r = l + (r-l+1)/3*2;
    if(new_r == (int)answer.size())new_r--, new_l--;
    communicate(answer[new_l], answer[new_r], new_el);
    if(res == new_el)return find_index(new_el, new_l, new_r, answer);
    if(res == answer[new_l])return find_index(new_el, l, new_l, answer);
    return find_index(new_el, new_r, r, answer);
}
// 18, 32, 35, 28, 13, 45, 38, 10, 21, 23, 25, 
// 27, 36, 14, 33, 5, 43, 34, 40, 46, 42, 15, 
// 20, 19, 11, 8, 12, 1, 24, 17, 9, 50, 37, 39, 
// 48, 29, 6, 7, 16, 30, 26, 31, 44, 47, 2, 3, 41, 4, 49, 22
int main(){
    // fastIO;
    int t, n, q;
    cin>>t>>n>>q;
    ...
    while(t--){
        ...
        communicate("1 2 3");
        swap(answer[res-1], answer[1]);
        forii(4, n+1){
            // find where to insert the new element
            int l = -1;
            int r = answer.size();
            int index = find_index(i, l, r, answer);
