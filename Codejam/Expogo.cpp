...
int main(){
    ...
    fork1(t){
        ...
        while(x || y){
            ...
            // choose which one performs jump
            if(x & 1){
                // choose direction
                int x1 = x + 1, x2 = x - 1;
                if((x1>>1) == 0 && (y>>1) == 0)x = x1, ans += 'W';
                else if((x2>>1) == 0 && (y>>1) == 0)x = x2, ans += 'E';
                else if(x1 & y & 2)x = x2, ans += 'E';
                else if(((x1 | y) & 2) == 0)x = x2, ans += 'E';
                else x = x1, ans += 'W';
            }
            else{
                cerr<<x<<" "<<y<<endl;
                int y1 = y + 1, y2 = y - 1;
                if((x>>1) == 0 && (y1>>1) == 0)y = y1, ans += 'S';
                else if((x>>1) == 0 && (y2>>1) == 0)y = y2, ans += 'N';
                else if(y1 & x & 2)y = y2, ans += 'N';
                else if(((y1 | x) & 2) == 0)y = y2, ans += 'N';
                else y = y1, ans += 'S';
            }
            x>>=1;
            y>>=1;
