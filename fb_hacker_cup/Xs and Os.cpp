...
int main(){
    ...
    fork1(t){
        ...
        vector<int> row_pos(n, true), col_pos(n, true);
        ...
        int amount = n+1;
        fori(n){
            if(row_pos[i]) amount = min(amount, n - row_sum[i]);
        }
        fori(n){
            if(col_pos[i]) amount = min(amount, n - col_sum[i]);
        }
        if(amount == n+1){
        ...
        if(amount == 1){
            ...
            fori(n){
                forj(n){
                    if(table[i][j] == '.' && (col_sum[j] == n-1 || row_sum[i] == n-1)) answer += 1;
                }
            }
        }
        else{
            fori(n){
                if(row_pos[i] && n - row_sum[i] == amount) answer += 1;
            }
            fori(n){
                if(col_pos[i] && n - col_sum[i] == amount) answer += 1;
            }
        }
        cout << answer << endl;
    }
