...
int max_value(int cur, int prev, vector<int>& gold, vector<vector<int>>& E) {
    int answer = 0;

    for(auto &i: E[cur]){
        if(i == prev)
            continue;
        answer = max(answer, max_value(i, cur, gold, E));
    }
...
int best_value(vector<int>& gold, vector<vector<int>>& E){
    vi best = {0,0};

    for(auto &i: E[1]){
        int sub_best = max_value(i, 1, gold, E);
        // debug(sub_best);
        if(sub_best > best[0])best[0] = sub_best;
        if(best[0] > best[1]) swap(best[0], best[1]);
    }
    // debug(cur, prev, sum, best);
    return best[0] + best[1] + gold[1];
}
