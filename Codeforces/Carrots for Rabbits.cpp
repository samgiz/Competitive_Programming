...
ll score(int n, int k){
    if(n <= k)return n;
    ll rem = n % k;
    ll q = n / k;
    return (q+1) * (q+1) * (rem) + q * q * (k - rem); 
}

ll increase(Carrot c){
    return score(c.n, c.slices) - score(c.n, c.slices+1);
}

bool operator<(const Carrot &c1, const Carrot &c2)
{
    return increase(c1) < increase(c2);
}
...
int main(){
    ...
    while(Q.size()){
        auto t = Q.top();
        Q.pop();
        debug(t.n, t.slices, score(t.n, t.slices));
        ans += score(t.n, t.slices);
    }
    cout<<ans<<endl;
}
