...
void reca(int a, int cur){
    if(A.count(a))return;
    A[a] = cur;
    if(a%5 == 0)reca(a/5, cur+1);
    if(a%3 == 0)reca(a/3, cur+1);
    if(a%2 == 0)reca(a/2, cur+1);
}

void recb(int b, int cur){
    if(B.count(b))return;
    B[b] = cur;
    if(b%5 == 0)recb(b/5, cur+1);
    if(b%3 == 0)recb(b/3, cur+1);
    if(b%2 == 0)recb(b/2, cur+1);
}
