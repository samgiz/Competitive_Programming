...
std::ostream& operator<< (std::ostream& out, const std::vector<T>& v) {
  if ( !v.empty() ) {
    std::copy (v.begin(), v.end_game(), std::ostream_iterator<T>(out, " "));
  }
  return out;
}
template <typename T>
...
void end_game(string s){
	cout<<s;
	exit(0);
}
bool filled_row(int i){
	return T[i][0]==T[i][1] && T[i][0]==T[i][2] && (T[i][0]=='0' || T[i][0]=='X');
}
bool filled_column(int i){
	return T[0][i]==T[1][i] && T[0][i]==T[2][i] && (T[0][i]=='0' || T[0][i]=='X');
}
...
int main(){
	...
	if(xs > os + 1 || os > xs)end_game("illegal");
	fori(3){
		if(filled_row(i) && filled_row((i+1)%3))
			end_game("illegal");
	}
	fori(3){
		if(filled_column(i) && filled_column((i+1)%3))
			end_game("illegal");
	}
	fori(3){
		if(filled_row(i) && T[i][i] == 'X'){
			if(xs == os)end_game("illegal");
			end_game("the first player won");
		}
	}
	...
	fori(3){
		if(filled_column(i) && T[i][i] == 'X'){
			if(xs == os)end_game("illegal");
			end_game("the first player won");
		}
	}
	fori(3){
		if(filled_row(i) && T[i][i] == '0'){
			if(xs != os)end_game("illegal");
			end_game("the second player won");
		}
	}

	fori(3){
		if(filled_column(i) && T[i][i] == '0'){
			if(xs != os)end_game("illegal");
			end_game("the second player won");
		}
	}
	...
	if(xs + os == 9)end_game("draw");
	if(xs == os)end_game("first");
	end_game("second");
}
