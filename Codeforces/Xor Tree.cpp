...
struct Trie{
    ...
    void add(int val){
        ...
        fori_(32){
            if((1<<i) & val){
                // add to bucket
                if(!buckets.count(i))buckets[i] = new Trie();
                buckets[i]->add(val ^ (1<<i));
                return;
            }
        }
    }
    int calc(){
        int ans = sz - buckets.size();
        if(ends_here)ans -= 1;
        debug("initial ans", ans, sz, ends_here, buckets.size());
        bool fst = !ends_here;
        int smaller = 0;
        for(auto &i: buckets){
            auto bucket = i.se;
            debug("bucket", i.fi);
            ans = min(ans, (int)(sz - (bucket->sz - bucket->calc()) - (buckets.size() + ends_here - 1 - smaller)));
            debug("new size", ans, sz, ends_here);
            if(fst){
                fst = false;
            }
            else{
