...
int main(){
    ...
    while(true){
        ...
        bool f11 = false;
        fori1(n){
            f00 |= !(Pts[i].x & 1) && !(Pts[i].y & 1);
            f10 |=  (Pts[i].x & 1) && !(Pts[i].y & 1);
            f11 |=  (Pts[i].x & 1) &&  (Pts[i].y & 1);
            f01 |= !(Pts[i].x & 1) &&  (Pts[i].y & 1);
        }
        // cerr<<f00<<f11<<f01<<f10<<endl;
        if((f00 || f11) && (f01 || f10)){
            vi Ans;
            fori1(n){
                if((Pts[i].x + Pts[i].y) & 1)Ans.pb(i);
            }
            cout<<Ans.size()<<endl;
