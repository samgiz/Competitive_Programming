...
void calc_gcd(int cur, int prev){
    Ans[cur] = __gcd(Ans[prev], A[cur]);
    // cerr<<cur<<" "<<prev<<endl;
    for(auto &i: E[cur]){
        if(i == prev)continue;
        calc_gcd(i, cur);
...
void calc_div(int cur, int prev, int div, bool ch){
    if(A[cur]%div == 0){
        Ans[cur] = max(Ans[cur], div);
        for(auto &i: E[cur]){
            if(i == prev)continue;
            calc_div(i, cur, div, ch);
        }
    }
    else if(ch){
        Ans[cur] = max(Ans[cur], div);
...
int main(){
    ...
    fori1(A[1]){
        // cerr<<i<<endl;
        if(A[1]%i==0)calc_div(1, 0, i, true);
    }
    fori1(n)cout<<Ans[i]<<" ";
}
