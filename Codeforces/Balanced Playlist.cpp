...
int main(){
    ...
    forii_(mi+1, mi+n){
        // Calculate at i by using the info of the next element
        // Do this backwards
        while(Q.size() && (Q.top().fi*2 < A[i%n] || cur < Q.top().se)){
            // cout<<"qfi: "<<Q.top().fi<<endl;
            // cerr<<"cur: "<<cur<<" Qtopfi: "<<Q.top().fi<<" Qtopse: "<<Q.top().se<<endl;
            cur = min(cur, Q.top().se);
            Q.pop();
        }
        Ans[i%n] = cur - i;
        Q.push({A[i%n], i});
...
// Where pushed in
// Value
// Priority queue with min priority
// Priority for value