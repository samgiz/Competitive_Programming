...
int main(){
    ...
    while(cur_col >= 0 && A[cur_col] == 0)cur_col--;
    if(cur_col < 0){
        cout<<0;
        exit(0);
    }
    ...
    int cur_line = n;
    vpii availables;
    vpii availables_for_3;
    while(cur_col >= 0){
        if(A[cur_col] == 1){
            cur_line -= 1;
            Ans.pb({cur_line, cur_col});
            availables.pb({cur_line, cur_col});
        }
        else if(A[cur_col] == 2){
            ...
            pii available = availables.back();
            availables.pop_back();
            Ans.pb({available.fi, cur_col});
            availables_for_3.pb({available.fi, cur_col});
        }
        else if(A[cur_col] == 3){
            if(availables.size() == 0 && availables_for_3.size() == 0){
                cout<<-1;
                exit(0);
            }
            pii available;
            if(availables_for_3.size()){
                available = availables_for_3.back();
                availables_for_3.pop_back();
            }
            else{
                available = availables.back();
                availables.pop_back();
            }
            cur_line -= 1;
            Ans.pb({cur_line, cur_col});
            Ans.pb({cur_line, available.se});
            availables_for_3.pb({cur_line, cur_col});
        }
        // debug(availables);
        cur_col -= 1;
    }
