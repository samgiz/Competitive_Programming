...
int main(){
	...
	forg1(t){
		...
		int sumb = 0, sumdot = 0, sb = 0, sdot = 0;
		fori(ct){
			sumb += s[i]=='B';
			sumdot += s[i]=='.';
		}
		...
		if(sb && sdot && sb+sumb >=2){
			cout<<"Y"<<endl;
			continue;
		}
		if(ct == s.size()-1 || (ct == s.size()-3 && s[s.size()-2] == 'B' && s[s.size()-1] == '.') || (ct == s.size()-3 && s[s.size()-2] == '.' && s[s.size()-1] == 'B')){
			cout<<"Y"<<endl;
			continue;
		}
		if(sumb && sumdot && sb+sumb >=2){
			cout<<"Y"<<endl;
			continue;
		}
		cout<<"N"<<endl;
