...
ll recurse(ll larger, ll smaller, ll pos, ll a, ll b){
    debug(a, b);
    if(a == b)return 1;
    ll middle = (a + b) / 2;
    debug(middle, pos);
    if(middle == pos){
        ll i = recurse(larger, smaller, pos, middle + 1, b);
        debug("==", i);
        return i;
    }
    if(middle < pos){
        ll i = smaller * recurse(larger, smaller-1, pos, middle + 1, b) % M;
        debug("<", i);
        not_considered -= 1;
        return i;
    }
    ll i = larger * recurse(larger-1, smaller, pos, a, middle)%M;
        debug(">", i);
        not_considered -= 1;
        return i;
}
...
int main(){
    ...
    cin>>n>>x>>pos;
    not_considered = n-1;
    ll k = recurse(n-x, x-1, pos, 0, n);
    fori1(not_considered)k = (k * i) % M;
    cout<<k;
}
