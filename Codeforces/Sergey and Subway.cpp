...
pll recurse(int cur, int prev){
	int maxcur=0, maxnext=0;
	for(auto i: E[cur]){
		if(i==prev)continue;
		pll p = recurse(i, cur);
		maxcur ;
...
// distfromcur = amountof1neighbours + amountof2neighbours*2 + sumofdistfrom2neighbours
// amountofnext2neighbours = amountof1neighbours
// amountofnextoneneighbours++
// sumofnextdistfrom2neighbours = sum over all neighbours of sumofnextnextdistfrom2neighbours
// sumofnextnextdistfrom2neighbours += distfromcur
int main(){
	fastIO;
	int n;
	cin>>n;
