...
int main(){
	...
	fori1(n){
		if(P[i]==i){
			if(root==0)root=i;
			else changes++, P[i]=root;
		}
	}
	fori1(n){
		if(i==root)continue;
		int x = parent(i), y = parent(P[i]);
		if(x==y){
			changes++;
		...
		int z = parent(P[i]);
		if(z==x)continue;
		if(Rank[z]>Rank[x])Par[x]=z;
		else{
			if(Rank[z]==Rank[x])Rank[x]++;
			Par[z]=x;
		}
	}
	cout<<changes<<endl;
	fori1(n)cout<<P[i]<<" ";cerr<<endl;
	// fori1(n)cerr<<parent(i)<<" ";
}
