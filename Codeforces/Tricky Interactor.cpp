...
int main(){
	...
	if(n==1){
		...
		bool flipped1 = false, flipped2 = false, flipped3 = true;
		if(tp+t<n)Ans[1] = Ans[2] = 0;
		else if(tp+t>n)Ans[1]=Ans[2]=1;
		else{ // everything after 2 is flipped
			// flip 1 not 2
			t = tp;
			while(tp == t){
				cout<<"? "<<2<<" "<<3<<endl;
			...
			flipped1 = flipped2 = flipped3 = false;
		}
		t = tp;
		forii(2, n){
			flipped1 = flipped2, flipped2 = flipped3;
			flipped3 = !flipped3;
			while(tp == t){
				cout<<"? "<<i<<" "<<i+1<<endl;
				cin>>tp;
			...
			if(tp+t==n){
				Ans[i+1] = (Ans[i]^flipped1)^1^flipped2;
			}
			else if(tp+t<n){
				Ans[i+1] = 0^flipped2;
			}
