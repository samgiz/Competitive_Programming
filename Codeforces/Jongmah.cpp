...
bool isLegal(int g, int i, int j, int k){
	return A[g-2]>=i+k && A[g-1]>=j+k && A[g]>=k;
}
...
int main(){
	...
	Am[1][0] = Am[2][0] = 0;
	forgg(3, m+1){ // check if legal
		fori(5){
			forj(3){
				fork(3){
					if(isLegal(g, i, j, k) && Am[g-1][i*3+j]!=-1)
						Am[g][(j+k)*3+k]=max(Am[g][(j+k)*3+k], k+Am[g-1][i*3+j]+(A[g-2]-i-k)/3);
				}
			}
		}
	}
	...
	fori(5){
		forj(3){
			if(Am[m][i*3+j] != -1)
				ans = max(ans, Am[m][i*3+j]+(A[m-1]-i)/3+(A[m]-j)/3);
		}
	}
	cout<<ans;
}
