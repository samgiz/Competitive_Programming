...
int main(){
    ...
    for(int i=0; i<n; i++){
        vector<int>ans;
        // Continue if no change is needed
        if(end && C[n-1-i] == cop[i] || !end && C[i] == cop[i])
            continue;
        if(i == 0 && cop[0] == C[0]){
            end = 0;
            continue;
        ...
        // Find the next element to add, and figure out how to put it next to the tail
        int next = -1;
        debug(next, C);
        vector<int>nec;
        if(end){
            ...
            if(next != 0)ans.pb(next);
            if(n - next - i != 0)ans.pb(n - next - i);
            int ct = n-1;
            forj(i){
                // debug(ct);
                nec.pb(C[ct--]);
            ...
            ct = next;
            forj(n - next - i)nec.pb(C[ct++]);
            ct = 0;
            forj(next)nec.pb(C[ct++]);
            forj(i)ans.pb(1);
        }
        else{
            debug("else");
            ...
            // 1 1 1 1 (next-i+1) (n-next-1)
            forj(i)ans.pb(1);
            if(next-i+1 != 0)ans.pb(next-i+1);
            if(n - next - 1 != 0)ans.pb(n - next - 1);
            // next+1 ... n
            int ct = next+1;
            while(ct < n){
                // debug(ct);
            ...
            // i ... next + 1
            ct = i;
            while(ct < next + 1)nec.pb(C[ct++]);
            // i-1 ... -1
            ct = i-1;
            while(ct >= 0)nec.pb(C[ct--]);
        }
        C = nec;
