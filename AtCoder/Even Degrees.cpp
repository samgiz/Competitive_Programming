...
vector<int>E[N];
vector<pii>Ans;
bool Vis[N], Finished[N];

bool generate_edges(int cur, int prev){
	int edges = 0;
	Vis[cur] = true;
	...
	for(auto i: E[cur]){
		...
		else if(!Vis[i] && generate_edges(i, cur)){
			edges++;
			Ans.pb({cur, i});
		}
	}
...
int main(){
	...
	generate_edges(1, 0);
	cerr<<Ans.size()<<endl;
	fori(m)cout<<Ans[i].fi<<" "<<Ans[i].se<<endl;
}
