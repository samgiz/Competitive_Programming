...
int BSIndex(){
    ...
    for(int i=0; i<=a; i++)Init[i] = i+1;
    for(int i=a+1; i<n; i++)Init[i] = i+n+1;
    cerr<<"exit"<<endl;
    return a;
}
...
int main(){
    ...
    cin>>n;
    int ind = BSIndex();
    // Set answer to the 2 middle elements
    Ans[ind] = init[0];
    Ans[ind+n] = Ans[ind]=='R'?'B':'R';
    // Find colours of all other elements in Init
    for(int i=0; i<n; i++){
        if(i == ind)continue;
        Init[ind] = inverse(Init[i]);
        query();
        Ans[Init[ind]-1] = t[0];
    }
    // Invert all elements
    for(int i=0; i<n; i++){
        Init[i] = inverse(Init[i]);
    }
    // Find remaining colours
    for(int i=0; i<n; i++){
        if(i == ind)continue;
        Init[ind] = inverse(Init[i]);
        query();
