...
void handle(int i, int v1, int v2){
    int j = b - i + 1;
    if(v1 == v2){
        i1 = i;
    }
...
int main(){
    ...
    for(int _=0; _<t; _++){
        int val1, val2;
        cout<<1<<endl;cin>>val1;
        cout<<b<<endl;cin>>val2;
        handle(1, val1, val2);
        for(int i=2; i<=b/2; i+=4){
            for(int j=0; j<4; j++){
                cout<<i+j<<endl;cin>>val1;
                cout<<b-i+1-j<<endl;cin>>val2;
                handle(i+j, val1, val2);
            }
            switcheroo();
        }
        print_answer();
