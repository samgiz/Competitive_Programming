...
void insertString(string& s, int cur, node*& n){
	if(n == 0)n = new node();
	n->cnt++;
	if(cur < 0)return;
	insertString(s, cur-1, n->A[s[cur]-'A']);
}

int searchTree(node* n){
	if(n == 0)return 0;
	int ct = 0;
	fori(26){
		ct += searchTree(n->A[i]);
	}
	if(n->cnt >= ct + 2 && n != root){
		// cerr<<ct<<" a"<<endl;
		return ct + 2;
	}
	return ct;
...
int main(){
	...
	fori1(t){
		...
		fori(n){
			string s;
			cin>>s;
			insertString(s, s.size()-1, root);
		}
		cout<<searchTree(root)<<endl;
	}
}
