...
struct dest{
	...
	bool operator<(const dest& a) const{
		return a.w<w;
	}
};
...
int main(){
	...
	priority_queue<dest>Q;
	Q.push(dest(0, A[1], 1));
	while(Q.size()){
		dest t = Q.top();
		Q.pop();
		if(Vis[t.ind][t.fish])continue;
		Best[t.ind][t.fish] = t.w;
		Vis[t.ind][t.fish] = true;
		for(auto i: E[t.ind]){
			if(Vis[i.fi][t.fish|A[i.fi]])continue;
			Q.push(dest(t.w+i.se, t.fish|A[i.fi], i.fi));
		}
	}
	int ans = 1000000;
	fori(1<<k){
		forj(1<<k){
			if(Vis[n][i] && Vis[n][j] && (i|j) == (1<<k)-1)ans = min(ans, max(Best[n][i], Best[n][j]));
		}
	}
	cout<<ans;
}
