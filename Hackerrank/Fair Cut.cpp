...
ll A[N];
// Index, # of elements assigned to A
ll Sum[N][N];
...
int main(){
    ...
    fori(n)cin>>A[i];
    sort(A, A+n);
    // Set all the values to infinity initially
    fori(n+1){
        forj(k+1)Sum[i][j] = INF;
    }
    Sum[0][0] = 0;
    fori(n){//index
        forj(k+1){
            // Assign the element to B
            // To the left: j As
            // To the right: k-j
            // cerr<<i<<" "<<j<<" "<<i-j-n+k+i-j<<" "<<(2*j-k)<<" "<<Sum[i][j] + (2*j-k)*A[i]<<" "<<Sum[i][j] + (i-j-n+k+i-j)*A[i]<<endl;
            Sum[i+1][j] = min(Sum[i][j] + (2*j-k)*A[i], Sum[i+1][j]);
            // Assign the element to A
            // overall n-k
            // negative i-j
            Sum[i+1][j+1] = min(Sum[i][j] + (i-j-n+k+i-j)*A[i], Sum[i+1][j+1]);
        }
    }
    cout<<Sum[n][k]<<endl;
}
