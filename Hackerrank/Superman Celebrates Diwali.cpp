...
int DP[N][N]; // house, starting height
int max1[N], max2[N]; // max and second max DP values at certain height
...
int main(){
	...
	fori1(h){
		forj(n){
			if(i<I)DP[j][i]+=DP[j][i-1];
			else{
				if(DP[j][i-I]==max1[i-I])DP[j][i]+=max(DP[j][i-1], max2[i-I]);
				else DP[j][i]+=max(DP[j][i-1], max1[i-I]);
			}
		}
		max1[i]=max(DP[0][i], DP[1][i]);
		max2[i]=min(DP[0][i], DP[1][i]);
		forjj(2, n){
			if(DP[j][i]>max1[i])max2[i]=max1[i], max1[i]=DP[j][i];
			else if(DP[j][i]>max2[i])max2[i]=DP[j][i];
		}
	}
	// fori(h+1){
	// 	forj(n)cerr<<DP[j][i]<<" ";cerr<<endl;
	// }
	cout<<max1[h];
}
