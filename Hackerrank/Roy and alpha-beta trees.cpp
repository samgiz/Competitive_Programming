...
ll A[N];
ll alpha, beta;
ll Alpha[N][N], Beta[N][N], Num[N][N];
bool calculated[N][N];

void calculate(int a, int b){
    if(calculated[a][b])return ;
    Num[a][b] = 0;
    Alpha[a][b] = 0;
    Beta[a][b] = 0;
    if(a == b){
    ...
    for(int i=a; i<b; i++){
        calculate(a, i);
        calculate(i+1, b);
        Alpha[a][b] += (Beta[a][i]*Num[i+1][b]%M + Beta[i+1][b]*Num[a][i]%M + A[i]*Num[a][i]%M*Num[i+1][b]%M)%M;
        Beta[a][b] += (Alpha[a][i]*Num[i+1][b]%M + Alpha[i+1][b]*Num[a][i]%M)%M;
        Num[a][b] += Num[a][i] * Num[i+1][b]%M;
    }
    Alpha[a][b]%=M;
    Beta[a][b]%=M;
    Num[a][b]%=M;
...
int main(){
    ...
    while(t--){
        ...
        sort(A, A+n);
        calculate(0, n);
        cout<<(Alpha[0][n]*alpha%M - Beta[0][n]*beta%M + M)%M<<endl;
    }
}
