...
int main(){
	...
	while(t--){
		...
		sort(A, A+n, [](Domino a, Domino b){return a.x>b.x;});
		int ct = 0;
		// domino, kiek nuverčia
		stack<Domino>S;
		fori(n){
			A[i].ans = 1;
			while(S.size() && S.top().x<A[i].x+A[i].h)A[i].ans+=S.top().ans, S.pop();
			S.push(A[i]);
		}
		sort(A, A+n, [](Domino a, Domino b){return a.ind<b.ind;});
		fori(n)cout<<A[i].ans<<" ";
		cout<<endl;
	}
}
