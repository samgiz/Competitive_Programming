...
const ll M = 1e9+7;
long double A[N];
vector<long double>V = {0};
int cnt=0;

long double value(long double x){
	long double sum = 0;
	fori(cnt)sum+=abs(x-A[i]);
	return sum;
}
...
int main(){
	...
	sort(V.begin(), V.end());
	int a = 0, b = V.size()-1;
	long double av = value(V[a]);
	long double bv = value(V[b]);
	while(a+1!=b){
		int mid = (a+b)/2;
		long double midv = value(V[mid]);
		if(av>bv)av = midv, a = mid;
		else bv = midv, b = mid;
	}
	cout<<fixed;
	cout<<setprecision(20)<<min(av, bv);
}
