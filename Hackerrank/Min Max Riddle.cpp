...
int main(){
	...
	fori(n){
		int pos = Min[i].se, val = Min[i].fi;
		auto tp = S.upper_bound(mp(pos+1, N));
		--tp;
		pii interval = *(tp);
		cerr<<interval.fi<<" "<<interval.se<<endl;
		Ct[interval.se-interval.fi+1]--;
		pii int1 = mp(interval.fi, pos-1), int2 = mp(pos+1, interval.se);
		S.erase(interval);
		if(int1.se>=int1.fi){
			S.insert(int1);
			Ct[int1.se-int1.fi+1]++;
		}
		if(int2.se>=int2.fi){
			S.insert(int2);
			Ct[int2.se-int2.fi+1]++;
		}
		while(m>0 && Ct[m] == 0)Ans[m--] = val;
		fori(10)cerr<<Ct[i]<<" ";cerr<<endl;
		cerr<<int1.fi<<" "<<int1.se<<"  "<<int2.fi<<" "<<int2.se<<endl<<endl;
	}
	fori1(n)cout<<Ans[i]<<" ";
}
