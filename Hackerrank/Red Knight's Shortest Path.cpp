...
int main(){
	...
	while(Q.size()){
		...
		// UL UR R LR LL L
		// -2,
		if(isLegal(f.fi-2, f.se-1)&&!Vis[f.fi-2][f.se-1])Q.push(mp(f.fi-2, f.se-1)), 
														 Vis[f.fi-2][f.se-1]=true, 
														 Par[f.fi-2][f.se-1] = f,
														 Val[f.fi-2][f.se-1] = "UL";
		if(isLegal(f.fi-2, f.se+1)&&!Vis[f.fi-2][f.se+1])Q.push(mp(f.fi-2, f.se+1)), 
														 Vis[f.fi-2][f.se+1]=true, 
														 Par[f.fi-2][f.se+1] = f,
														 Val[f.fi-2][f.se+1] = "UR";
		if(isLegal(f.fi, f.se+2)&&!Vis[f.fi][f.se+2])Q.push(mp(f.fi, f.se+2)), 
														 Vis[f.fi][f.se+2]=true, 
														 Par[f.fi][f.se+2] = f,
														 Val[f.fi][f.se+2] = "R";
		if(isLegal(f.fi+2, f.se+1)&&!Vis[f.fi+2][f.se+1])Q.push(mp(f.fi+2, f.se+1)), 
														 Vis[f.fi+2][f.se+1]=true, 
														 Par[f.fi+2][f.se+1] = f,
														 Val[f.fi+2][f.se+1] = "LR";
		if(isLegal(f.fi+2, f.se-1)&&!Vis[f.fi+2][f.se-1])Q.push(mp(f.fi+2, f.se-1)), 
														 Vis[f.fi+2][f.se-1]=true, 
														 Par[f.fi+2][f.se-1] = f,
														 Val[f.fi+2][f.se-1] = "LL";
		if(isLegal(f.fi, f.se-2)&&!Vis[f.fi][f.se-2])Q.push(mp(f.fi, f.se-2)), 
														 Vis[f.fi][f.se-2]=true, 
														 Par[f.fi][f.se-2] = f,
														 Val[f.fi][f.se-2] = "L";
	}
	while(Val[ie][je].size()){
		Ans.pb(Val[ie][je]);
		tie(ie, je) = make_tuple(Par[ie][je].fi, Par[ie][je].se);
	}
	if(ie==is&&je==js){
