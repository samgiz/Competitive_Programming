#include <bits/stdc++.h>

using namespace std;

#define mp make_pair
#define pb push_back
#define fi first
#define se second

typedef int def; //default parameter for loops

#define fori(a) for(def i=0; i<(a); i++)
#define fori1(a) for(def i=1; i<=(a); i++)
#define fori_(a) for(def i=(a)-1; i>=0; i--)
#define fori1_(a) for(def i=(a); i>0; i--)
#define forii(a, b) for(def i=(a); i<(b); i++)
#define forii_(a, b) for(def i=(b)-1; i>=(a); i--)

#define forj(a) for(def j=0; j<(a); j++)
#define forj1(a) for(def j=1; j<=(a); j++)
#define forj_(a) for(def j=(a)-1; j>=0; j--)
#define forj1_(a) for(def j=(a); j>0; j--)
#define forjj(a, b) for(def j=(a); j<(b); j++)
#define forjj_(a, b) for(def j=(b)-1; j>=(a); j--)

#define fork(a) for(def k=0; k<(a); k++)
#define fork1(a) for(def k=1; k<=(a); k++)
#define fork_(a) for(def k=(a)-1; k>=0; k--)
#define fork1_(a) for(def k=(a); k>0; k--)
#define forkk(a, b) for(def k=(a); k<(b); k++)
#define forkk_(a, b) for(def k=(b)-1; k>=(a); k--)

#define forl(a) for(def l=0; l<(a); l++)
#define forl1(a) for(def l=1; l<=(a); l++)
#define forl_(a) for(def l=(a)-1; l>=0; l--)
#define forl1_(a) for(def l=(a); l>0; l--)
#define forll(a, b) for(def l=(a); l<(b); l++)
#define forll_(a, b) for(def l=(b)-1; l>=(a); l--)

#define forg(a) for(def g=0; g<(a); g++)
#define forg1(a) for(def g=1; g<=(a); g++)
#define forg_(a) for(def g=(a)-1; g>=0; g--)
#define forg1_(a) for(def g=(a); g>0; g--)
#define forgg(a, b) for(def g=(a); g<(b); g++)
#define forgg_(a, b) for(def g=(b)-1; g>=(a); g--)

#define forf(a) for(def f=0; f<(a); f++)
#define forf1(a) for(def f=1; f<=(a); f++)
#define forf_(a) for(def f=(a)-1; f>=0; f--)
#define forf1_(a) for(def f=(a); f>0; f--)
#define forff(a, b) for(def f=(a); f<(b); f++)
#define forff_(a, b) for(def f=(b)-1; f>=(a); f--)

#define forh(a) for(def h=0; h<(a); h++)
#define forh1(a) for(def h=1; h<=(a); h++)
#define forh_(a) for(def h=(a)-1; h>=0; h--)
#define forh1_(a) for(def h=(a); h>0; h--)
#define forhh(a, b) for(def h=(a); h<(b); h++)
#define forhh_(a, b) for(def h=(b)-1; h>=(a); h--)

#define fastIO ios_base::sync_with_stdio(0);cin.tie(0);cout.tie(0)
#define endl '\n'

typedef long long ll;
typedef vector<int> vi;
typedef vector<ll> vl;
typedef pair<int, int> pii;
typedef pair<ll, ll> pll;
typedef vector<pii> vpii;
typedef vector<pll> vpll;

template <typename T>
std::ostream& operator<< (std::ostream& out, const std::vector<T>& v) {
    if ( !v.empty() ) {
        std::copy (v.begin(), v.end(), std::ostream_iterator<T>(out, " "));
    }
    return out;
}
template <typename T>
std::ostream& operator<< (std::ostream& out, const std::set<T>& v) {
    if ( !v.empty() ) {
    std::copy (v.begin(), v.end(), std::ostream_iterator<T>(out, " "));
    }
    return out;
}

const int N = 3e5+10;
const ll M = 1e9+7;

ll dp[N][32], sum[N];
ll value;
ll skip;

ll answer(ll x, ll k){
    if(x == 0)return 1;
    if(x < 0 || k < 0)return 0;
    while((1<<k) > x)k--;
    // cerr<<"B "<<x<<" "<<k<<endl;
    if(((1<<(k+1))-1)*9 < x)return 0;
    if(dp[x][k] != 0)return dp[x][k];
    ll ans = 0;
    ll tmp = 0;
    for(int i=0; i<10; i++){
        ll tp = answer(x-(1<<k)*i, k-1);
        // if(x == 6)cout<<"A "<<i<<" "<<tp<<" "<<k<<endl;
        ans += tp;
    }
    dp[x][k] = ans;
    return ans;
}
void bs(ll x){
    ll a = -1, b = 299999;
    // the answer is in the interval (a, b]
    while(a < b-1){
        ll mid = (a + b) / 2;
        if(sum[mid] < x)a = mid;
        else b = mid;
    }
    value = b;
    if(b == 0)skip = 0;
    else skip = x - sum[b-1] - 1;
}
string ans;
bool print(ll x, ll skip, ll k, string letter, ll& prev){
    if(k == -1){
        if(skip == 0 && x == 0){
            ans += letter;
            return true;
        }
        else return false;
    }
    else prev+=dp[x][k];

    if(x < 0 || k < 0)return false;
    if(dp[x][k] == 0){
        return print(x, skip, k-1, letter+'0', prev);
    }
    if(skip >= dp[x][k])return false;
    ans+=letter;
    ll cur = 0;
    for(int i=0; i<10; i++){
        if(print(x - (1<<k) * i, skip-cur, k-1, to_string(i), cur))return true;
    }
    return false;
}

int main(){
    fastIO;
    

    // This part calculates the number of decibinary numbers equal to x, where the most significant decibit is k, for x up to 200000, k up to most significant bit (this is different for each x)
    ll cur = 0;
    for(int i=0; i < 300000; i++){
        cur += (answer(i, (ll)26));
    }
    // cerr<<cur<<endl;
    // cerr<<maxi<<endl;

    // This part calculates how many decibinary numbers there are in the list that are less or equal to x. This enables us to binary search what value of any element in the list given its index, and also get the amount of such numbers that need to be skipped
    sum[0] = 1;
    ll mb = 0;
    fori1(300000-1){
        // if(i<20)cerr<<i<<" "<<mb<<endl;
        if((1<<(mb+1))==i)mb++;
        sum[i] += dp[i][mb] + sum[i-1];
    }
    // fori(10)cerr<<sum[i]<<endl;
    // bs(11);
    int q;
    cin>>q;
    

    fori(q){
        ll x;
        cin>>x;
        if(x == 1){
            cout<<0<<endl;
            continue;
        }
        // binary search the value of the element in the list, and which element with such value it is. Store these values globally
        bs(x);
        // print the actual decibinary value, given the number of elements to skip
        ans = "";
        ll a;
        print(value, skip, 26, "", a);
        int j = 0;
        while(ans[j] == '0')j++;
        forii(j, ans.size())cout<<ans[i];
        cout<<endl;
    }
}
