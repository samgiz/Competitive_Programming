...
void DFS(int cur, int par){
	// Initial result (before considering any subtrees)
	DP[cur][E[cur].size()] = 1;
	// cerr<<cur<<" "<<E[cur].size()<<endl;
	for(auto i: E[cur]){
		if(i==par)continue;
		DFS(i, cur);
		// add results from subtree
		forj(n+1){
			New[j]=0;
		}
		forj1(n){
