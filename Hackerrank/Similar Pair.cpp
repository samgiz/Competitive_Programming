...
void add(int l, int r, int cur, int curl, int curr){
	// cerr<<l<<" "<<r<<" "<<cur<<" "<<curl<<" "<<curr<<endl;
	if(l<=curl && r>=curr) T[cur]++;
	else if(l>curr||r<curl)return;
	else {
		T[2*cur]+=T[cur];
		T[2*cur+1]+=T[cur];
		T[cur]=0;
		add(l, r, 2*cur, curl, (curl+curr)/2);
		add(l, r, 2*cur+1, (curl+curr)/2+1, curr);
	}
}

int get(int t, int cur, int curl, int curr){
	if(curl==curr){
		return T[cur];
	}
	else {
		...
		T[cur*2+1]+=T[cur];
		T[cur]=0;
		if(t<=(curl+curr)/2)return get(t, 2*cur, curl, (curl+curr)/2);
		else return get(t, 2*cur+1, (curl+curr)/2+1, curr);
	}
}
...
int n, k;
void DFS(int cur){
	int init = get(cur, 1, 1, n);
	for(auto i: E[cur])DFS(i);
	ans+=get(cur, 1, 1, n)-init;
	add(max(1, cur-k), min(n, cur+k), 1, 1, n);
	cerr<<cur<<endl;
	// fori1(n)cerr<<get(i, 1, 1, n)<<" ";cerr<<endl;
}
