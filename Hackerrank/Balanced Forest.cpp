...
ll recurse(int v, int par){
	ll am = Val[v];
	for(auto u: E[v]){
		if(u==par)continue;
		am += recurse(u, v);
	}
	if(sum<=3*am && S.count(am)){
		if(ans == -1)ans = 3*am-sum;
		else ans = min(3*am-sum, ans);
	}
	if(sum<=3*(sum-am+Val[v]) && S.count(sum-am+Val[v])){
		if(ans == -1)ans = 3*(sum-am+Val[v])-sum;
		else ans = min(3*(sum-am+Val[v])-sum, ans);
	}
	S.insert(am);
	return am;
}
