...
ll A[4][3001][4096]; // highest element (to guarantee increasingness), xor value
...
int main(){
	...
	sort(K, K+4);
	fori1(K[0]){ // xor value
		forjj(i, K[3]+1){ // all elements at most
			A[0][j][i]=1;
		}
	}
	ll ans = 0;
	fori1(3){
		forj(4096){ // previous xor value
			fork1(K[i]){ // next element
				if(A[i-1][k][j])cerr<<i<<" "<<k<<" "<<j<<" "<<endl;
				A[i][k][j^k] = A[i-1][k][j]; // max element equals k
			}
		}
		// make max element at most k
		forj(4096){
			fork1(K[3]){
				A[i][k][j] += A[i][k-1][j];
			}
