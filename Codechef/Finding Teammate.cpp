...
ll A[N];
ll Fit[N], Fac[N], Two[N]; // assume tight fit, and if not, then multiply by 2;
...
int main(){
	fastIO;
	Fac[0]=1;
	forii(1, N)Fac[i]=Fac[i-1]*i%M;
	Two[0]=1;
	forii(1, N)Two[i]=Two[i-1]*2%M;

	fori(N){
		Fit[i]=Fac[i]*inverse(Fac[i/2])%M*inverse(Two[i/2])%M; // n! / (n/2)! / 2^(n/2)
	}
	...
	fori(t){
		...
		while(s!=n){
			while(e<n&&A[s]==A[e])e++;
			if(e%2!=s%2 || s%2==0)ans=ans*Fit[e-s]%M;
			else ans=ans*Fit[e-s-2]%M*(e-s-1)%M*(e-s)%M;
			s=e;
		}
		cout<<ans<<endl;
	}
